...
class Snake:
    ...
    def reset(self):
        ...
        self.death = False
        self.score = 0
        self.apples = []  # リンゴのリストを初期化
        self.generate_apples(30)  # 初期リンゴを3個生成
    ...
    def check_apple(self):
        ...
        head = self.snake[0]
        for apple in self.apples[:]:  # リストのコピーをイテレート
            if head == apple:
                self.score += 1
                self.snake.append(self.popped_point)
                self.apples.remove(apple)  # 食べられたリンゴを削除
                self.generate_apple()  # 新しいリンゴを1つ生成
                pyxel.play(0, 0)

    def generate_apples(self, count):
        """Generate multiple apples randomly."""
        for _ in range(count):
            self.generate_apple()

    def generate_apple(self):
        """Generate a single apple randomly."""
        snake_pixels = set(self.snake)
        apple_pixels = set(self.apples)

        new_apple = self.snake[0]
        while new_apple in snake_pixels or new_apple in apple_pixels:
    ...
    def draw(self):
        """Draw the background, snake, score, and apples OR the end screen."""
...
def define_sound_and_music():
    ...
    pyxel.sounds[4].set(
        notes=("f0 r a4 r  f0 f0 a4 rf0 r a4 r   f0 f0 a4 f0"),
        tones="n",
